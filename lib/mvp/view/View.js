...
import {
  HTMLLiteralToHTMLTemplate,
  HTMLTemplateContent,
} from "./render/startRender"
...
export class View extends Presenter {
   ...
   * @param {Template | String} param.template
   */
  ...
  #temp(template) {
    ...
    if (template instanceof Template) {
      ...
      for (
        let index = 0;
        index < template.getHTML().childNodes.length;
        index++
      ) {
  ...
  async render() {
    ...
    const mode = { mode: "set" }
    await dataChangeStrategy[mode.mode].apply(this, [
      this.#nodeSymbol,
      this.#dataNodes,
    ])
  ...
  get templates() {
    if (!this.#templates) {
      this.#templates = new PropertyTemplateCollection()
    }
  ...
  addTemplate(template) {
    for (const name in template)
      this.#templates[name] = this.#temp(template[name])
  }
...
const dataChangeStrategy = {
  set: updateAllExistingNodes,
  add: generateNewNodes,
  filter: updateAllExistingNodes,
  clear: clearDataNodes,
  delete: deleteDataNodes,
}
...
async function clearDataNodes(data, dataNodes) {
  for (const entry of dataNodes) {
    dataNodes.delete(entry[0])
    await removeDataNodes(entry[1])
  }
...
function randomUUID() {
  if (self.crypto.randomUUID) return self.crypto.randomUUID().substring(24)
  if (self.crypto.getRandomValues)
    return self.crypto.getRandomValues(new Uint32Array(1))[0].toString()
...
async function generateNewDataNodes(data, dataNodes, renderFunction) {
  const renderNewNodes = async (item) => {
    if (!(typeof item == "symbol") && !item[Symbol.for("item")])
      item[Symbol.for("item")] = randomUUID()
