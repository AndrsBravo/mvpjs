...
export class View extends Presenter {
  ...
  async #updateAllExistingNodes(data) {
    ...
    const nodesAdded = await globalRender(this.#page._page, this, this.getTemplate(), this._target, data, this.#nodes);
  ...
  select() {
    ...
    this.#nodes.forEach(element => element.classList.toggle("selected"));
  }
