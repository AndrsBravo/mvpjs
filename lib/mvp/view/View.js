...
function randomUUID() {
  ...
  if (self.crypto.getRandomValues) return self.crypto.getRandomValues(new Uint32Array(1))[0].toString()
...
async function generateNewDataNodes(data, dataNodes, renderFunction) {
  ...
  const renderNewNodes = async (item) => {
    ...
    if (!(typeof (item) == "symbol") && !item[Symbol.for("item")]) item[Symbol.for("item")] = randomUUID();
