const argsFormat = (template) => {
  ...
  function directive(property) {
    ...
    if (/\w+\s+in\s+\w+/.test(value))
      value = `$${value.replace(value, setIn(clearedValue))}`

    if (/\w+\s+of\s+\w+/.test(value))
      value = `$${value.replace(value, setOf(clearedValue))}`

    if (/\w+\s+as\s+\w+/.test(value))
      value = `$${value.replace(value, setAs(clearedValue))}`
  ...
  function setIn(params) {
    ...
    if (!value.includes("data.") && !value.includes("meta."))
      value = `'${value.trim()}'`
