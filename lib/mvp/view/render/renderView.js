...
export default async function (view) {
  ...
  for (const section of listOfChildrenSections) {
    await setElementAsSection.apply(section, [view])
  }
  ...
  for (const child of listOfChildrenWithForPropertySet)
    setForProperty.apply(child, [view])

  if (this.matches("[list]")) setListProperty.apply(this, [view])
  const listOfChildrenWithListPropertySet = this.querySelectorAll("*[list]")
  for (const child of listOfChildrenWithListPropertySet)
    setListProperty.apply(child, [view])
}

async function setElementAsSection(view) {
  const sectionId = (
    (view.page.name ? `${view.page.name}_${view.name}_` : view.name + "_") +
...
function setElementId(view) {
  ...
  if (this.matches("[data-section]")) {
    object.id = this.dataset.section
    view.ids.add({ ...object })
