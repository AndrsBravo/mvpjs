...
const updateNodesRender = async function (
  metaData,
  view,
  template,
  target,
...
const globalRender = async function (
  metaData,
  view,
  template,
  target,
  ...
  if (data.length > nodes.length) {
    ...
    const adicionados = await addNodesRender(
      metaData,
      view,
      template,
      target,
...
export function HTMLTemplateContent(template) {
  if (template.content.children[0] instanceof HTMLTemplateElement)
    return HTMLTemplateContent(template.content.children[0])
  return template
}
