export function renderSys(htmlElemet, presenter) {
    ...
    for (const property in htmlElemet.dataset) {
        ...
        if (Object.hasOwnProperty.call(htmlElemet.dataset, property)
    ...
    events.forEach(element => sysRenderStrategy["sysEvent"](element, presenter));
}
...
const sysRenderStrategy = {
    ...
    sysEvent: (el, presenter) => {
        ...
        events.forEach(event => sysRenderStrategy["callEvent"](event, el, presenter));
