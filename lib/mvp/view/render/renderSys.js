...
export function renderSys(htmlElemet, presenter) {
  ...
  if (htmlElemet.matches("a[href]")) { event(el); }
  ...
  function event(el) {
    el.addEventListener("click", (evt) => {
      evt.preventDefault();
      evt.stopPropagation();
      history.pushState({ page: evt.currentTarget.href }, "", evt.currentTarget.href);
      //history.replaceState({ page: evt.currentTarget.href }, "", evt.currentTarget.href);
      System.routes();
    });
  }
