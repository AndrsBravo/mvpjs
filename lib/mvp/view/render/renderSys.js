import addEventStrategy from "lib/core/system/event-handlers/addEventStrategy.js"
/**
 *
...
export default function (view) {
  ...
  if (this.matches(linkHrefSelectorCondition)) {
    eventToALink.apply(this, [page])
    return
  }
  ...
  for (const property of eventsPropertiesSet) {
    if (this.matches(`[${property}]`))
      addEventStrategy.apply(this, [page, property])

    const elements = this.querySelectorAll(`[${property}]`)

    elements.forEach((element) =>
      addEventStrategy.apply(element, [page, property]),
    )
  }
}

function eventToALink(page) {
  if (this.matches("[data-ignore]")) {
    return
  }
  addEventStrategy.apply(this, [page])
}
