...
export default function (el, view, item) {
  ...
  if (el.matches("*[class*='{']")) {
    dataRender["class"](el, item);
  }

  if (el.matches("*[id*='{']") && el.id.indexOf("{") > -1) {dataRender["id"](el, item); };

  var target = el.querySelectorAll("*[target*='{']");
  var clases = el.querySelectorAll("*[class*='{']");
  ...
  var dataModel = el.querySelectorAll("*[data-model]");
  ...
  ids.forEach((ele) => {  if (ele.id.indexOf("{") > -1) dataRender["id"](ele, item) });
  clases.forEach((el) => dataRender["class"](el, item));

  if (el.dataset.model && el.dataset.template) {
    el.appendChild(generateView(view,
      view.templates()[el.dataset.template],
      item[el.dataset.model])
    )

  }

  dataModel.forEach(ele => {

    ele.appendChild(generateView(view,
      view.templates()[ele.dataset.template],
      item[ele.dataset.model])
    )
...
function algo() {
  ...
  dataModel.forEach(function (ele) {
    item[ele.dataset.model];
