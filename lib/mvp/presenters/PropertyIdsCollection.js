  ...
  #proxy(object) {
    const handler = {
      get(target, prop) {
        ...
        if (prop == "addEventListener") {
          ...
          return async (...args) => {
            ...
            const eventHandler = await this.handler(
              target,
              eventName,
              eventListener,
            )
        ...
        if (typeof target[prop] == "function" && !prop.startsWith("on"))
          return (param) => target[prop].call(target, [param])
      ...
      async set(target, prop, value) {
        if (prop == "id")
          throw `Changing '${prop}' attribute at this context is not allowed!`
        ...
        if (prop.startsWith("on")) {
          if ((!prop) in target)
            throw `${target.tagName} does not have ${prop} event`
      ...
      async handler(target, eventName, eventListener) {
        ...
        const eventManager = await eventHandlers[event](
          eventListener,
          object.scope,
          eventDelayMode(target, `on${eventName}`),
        )
        return eventManager
      },
    }
