  ...
  #dataProxyHandler(model) {
    return {
      get(target, prop, receiver) {
        const extractField = (letra) =>
          letra.charAt(0).toLowerCase() + letra.slice(1)
        ...
        if (prop == "reload")
          return () => {
            model.callListeners({ set: target })
          }
        if (prop == "last")
          return () => {
        ...
        if (prop == "filter")
          return (fn) => {
            const data = target.filter(fn)
            model.callListeners({
              filter: data,
            }) /*/model.callListeners(target)*/
          }
        if (prop == "clearFilter")
          return () => {
            model.callListeners({ set: target })
        ...
        if (prop == "clear")
          return () => {
            ;(target = []), model.callListeners({ clear: "" })
          }

        if (prop in target) return target[prop]
        if (target.length && Object.hasOwnProperty.call(target[0], prop)) {
        ...
        if (prop === "set")
          return async (data) => {
            ...
            if (data instanceof Array) {
              ...
              return await model.callListeners({ set: target })
            }
            target.push(data)
            return await model.callListeners({ set: target })
          }
        ...
        if (prop.startsWith("findBy"))
          return (data) => {
            const field = extractField(prop.replace("findBy", ""))
            return target.find((item) => item[field] == data)
          }
        if (prop.startsWith("filterBy"))
          return (data) => {
            const field = extractField(prop.replace("filterBy", ""))
            receiver.filter((item) => item[field] == data)
          }
        if (prop.startsWith("sortBy"))
          return () => {
            const field = extractField(prop.replace("sortBy", ""))
            return target.sort((item, item2) => item[field] - item2[field])
          }

        if (prop.startsWith("mapAll"))
        ...
        if (prop.startsWith("map"))
          return () => {
            const field = extractField(prop.replace("map", ""))
            return target.map((item) => {
