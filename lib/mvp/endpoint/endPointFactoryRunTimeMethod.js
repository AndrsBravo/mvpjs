...
function prepareEndPointData(data) {
  if (
    ...
    this.requestInit.headers["Content-Type"].startsWith("application/json")
  ) {
...
function defineEnPointParams(data) {
  ...
  if (paramsProvided < paramsExpected) {
    throw Error(
      `Path param missed '${url}' ${paramsExpected} 'expected' and ${paramsProvided} 'provided' ${JSON.stringify(data)}`,
    )
  }
...
async function fetchEndPoint() {
  ...
  if (resType.indexOf("text/plain") > -1) {
