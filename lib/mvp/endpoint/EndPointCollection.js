...
import {
  defineEndPointRequestInit,
  defineEndPointUrl,
} from "./endPointFactoryMethod"
...
function defineEndPointsConfiguration(endPointCollection) {
  for (const endPointIdentifier in this.endPoints) {
    ...
    Object.defineProperty(endPointCollection, endPointIdentifier, {
      value: () => model,
      writable: false,
    })
  }
...
function serverEvent(request, response) {
  if (!response.sse) {
...
webserver.get("/news/events", serverEvent)
