...
async function addEventStrategy(page, globalEventHandlerType) {
  ...
  if (this.matches("a[href]")) {
    const eventsManager = await eventHandlers.link(
      eventDelayMode(this, "onclick"),
    )
  ...
  if (!page[eventObject.listener]) {
    //  console.warn("La pagina: ", page.name);
    console.warn(
  ...
  const eventTypes = [
    `${this.type || this.tagName.toLowerCase()}${eventObject.eventType}`,
  ...
  const eventManager = await eventHandlers[eventObject.eventType](
    page[eventObject.listener],
    page,
    eventDelayMode(this, eventObject.eventName),
  )

  if (this[eventObject.eventName])
    this[eventObject.eventName] = eventManager.listener
}
