...
export default class {
  ...
  async #addRoute(location) {
    ...
    if (!this.#layouts.has(routeObj.path)) {
      const { default: RouteLayout } = routeObj.route.layout
        ? await routeObj.route.layout()
        : { default: null }
      this.#layouts.set(
        routeObj.path,
        RouteLayout ? new RouteLayout() : new Layout({ name: "body_layout" }),
      )
      await this.#layouts.get(routeObj.path).render()
      await this.#layouts.get(routeObj.path).start()
    }
