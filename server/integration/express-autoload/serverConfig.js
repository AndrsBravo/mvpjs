...
function scanViews(item) {
  ...
  if (views[name]) {
    ...
    console.warn(
      head,
      "Key access for the file:",
      chalk.gray(filePath.replace(process.cwd(), "")),
    )
  ...
  views[name] = ""
  viewsArray.push(
    `${newName}:{ file: "${item.name}", name:"${name}", path:"${filePath}" }`,
  )
...
export function getServerViews({ viewsPath, viewsFile, viewsFilePath }) {
  ...
  fs.writeFileSync(
    join(viewsFilePath, viewsFile),
    `export default { ${views.join(",")}  };`,
  )
}
...
function devOrProdTest(content) {
  ...
  return (
    (TEST_PRODUCTION_REGEXP.test(content)
      ? process.env.NODE_ENV?.toLocaleLowerCase() == "production"
      : true) &&
    (TEST_DEVELOPMENT_REGEXP.test(content)
