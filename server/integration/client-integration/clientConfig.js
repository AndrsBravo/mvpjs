...
import { scanDir } from "../express-autoload/scanner.js"
/**
 *
 * @param {Object} param
...
export async function setClientConfig({
  ...
  if (!modules || modules.length == 0) {
    console.log("No se encontraron módulos en la ruta cliente \n", clientPath)
    return
  }
  ...
  for (let module of modules) {
    ...
    const TEST_LAYOUT_REGEXP =
      /@layout[\s]*[\r]*\([\s]*['|"]{0,1}[\/]{0,1}([\w\/]*)['|"]{0,1}[\s]*[\r]*\)/i
    const TEST_PAGE_REGEXP =
      /@page[\s]*[\r]*\([\s]*['|"]{0,1}[\/]{0,1}([\w\/]*)['|"]{0,1}[\s]*[\r]*\)/i
