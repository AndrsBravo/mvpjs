...
import defaultConfig from "lib/application/config/default.config.js"
...
export async function init(options) {
  ...
  function setScripts() {
    ...
    for (const key in defaultConfig.system.scrips) {
      packageJson.scripts[key] = defaultConfig.system.scrips[key]
    }
  ...
  async function start() {
    return setUp({
      templateDirectory: options.templateDirectory + "/start",
      copyTo: options.cwd,
    })
  }
  ...
  async function frontEnd() {
    return setUp({
      templateDirectory: options.templateDirectory + "/src",
      copyTo: options.cwd + "/src",
    })
  }

  async function backEnd(o) {
    await setUp({
      templateDirectory: options.templateDirectory + "/backend",
      copyTo: options.cwd + "/backend",
    })
    setUp({
      templateDirectory: options.templateDirectory + `/${o.server}`,
      copyTo: options.cwd + "/backend/server",
    })
  }

  async function frontClient(o) {
    await setUp({
      templateDirectory: options.templateDirectory + "/frontend",
      copyTo: options.cwd + "/frontend",
    })
    setUp({
      templateDirectory: options.templateDirectory + "/src",
      copyTo: options.cwd + "/frontend/client",
    })
  }
