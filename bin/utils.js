...
export async function readingTemplateContent(options) {
  try {
    options.templateContent = fs.readFileSync(options.templateDirectory, {
      encoding: "utf-8",
    })
...
export async function readingHtmlFileContent(options) {
  ...
  options.templateContent = options.templateContent.replace(
    'html("")',
    html.template,
  )
  options.templateContent = options.templateContent.replace(
...
export async function fileTesting(options) {
  ...
  if (fileExists) {
    ...
    throw new Error(
      `%s The file ${options.value} for ${options.target} already exists`,
      chalk.bgRed.bold(" FILE EXISTS "),
    )
  }
...
export function updateConfigResource(options) {
  options.mvpConfigContent.routes.push(
    "./" +
      path
...
export function updateConfigLayout(options) {
  ...
  const layoutPlaceInConfigContent = options.mvpConfigContent.substring(
    options.mvpConfigContent.indexOf("layout"),
    options.mvpConfigContent.indexOf("}"),
  )

  options.mvpConfigContent = options.mvpConfigContent.replace(
    layoutPlaceInConfigContent,
    layout,
  )
}
...
export function updateConfigRoute(options) {
  ...
  const indexOfRouteAtConfigContent =
    options.mvpConfigContent.indexOf(routeName)

  if (indexOfRouteAtConfigContent > -1) {
    route = `${options.name}: () => import("${routePath}")`

    const indexOfEndRouteAtConfigContent = options.mvpConfigContent
      .substring(indexOfRouteAtConfigContent)
      .indexOf(",")

    routeToReplaceInConfigContent = options.mvpConfigContent.substring(
      indexOfRouteAtConfigContent,
      indexOfRouteAtConfigContent + indexOfEndRouteAtConfigContent,
    )
  }

  options.mvpConfigContent = options.mvpConfigContent.replace(
    routeToReplaceInConfigContent,
    route,
  )
}
...
export function setEndPointCollectionToaModel(options) {
  ...
  options.templateContent =
    `import ${options.belong} from "../${options.belong}.js"; ` +
    options.templateContent.replaceAll("EndPointCollectionName", options.belong)
}

async function mvpConfig(options) {
  const configUrl = path.resolve(options.cwd, "mvp.config.js")
  const config = await import(`file:///${configUrl}`)
